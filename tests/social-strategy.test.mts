...
test("subscriptions have no trial, expire at the boundary, renew without losing paid time and upgrade immediately", () => {
  ...
  const first = nextSubscriptionPeriod(null, "solo", 1, new Date("2026-01-31T12:00:00Z"))
  ...
  const upgraded = nextSubscriptionPeriod(first, "studio", 3, new Date("2026-02-05T12:00:00Z"))
  assert.equal(upgraded.brandLimit, 3); assert.equal(upgraded.expiresAt, first.expiresAt)
  const renewed = nextSubscriptionPeriod(upgraded, "studio", 3, new Date("2026-02-06T12:00:00Z"))
  assert.equal(renewed.expiresAt, "2026-03-28T12:00:00.000Z")
  assert.throws(() => nextSubscriptionPeriod(upgraded, "solo", 1, new Date("2026-02-06T12:00:00Z")))
  assert.throws(() => nextSubscriptionPeriod(null, "agency", 100))
  assert.throws(() => nextSubscriptionPeriod(null, "custom", 10))
  const restored = nextSubscriptionPeriod(first, "solo", 1, new Date("2026-05-01T12:00:00Z"))
