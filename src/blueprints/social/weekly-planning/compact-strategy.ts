...
export const COMPACT_STRATEGY_PROMPT = `Plan ONLY the current operational week under the approved persistent socialStrategy. Never replace the strategic objective at a calendar boundary or produce detailed future weeks. Explain in the weekly rationale what changed based on evidenceReview, what is observed vs hypothesized, and what remains unknown. If results are missing say so explicitly. Repeated strategic ideas across weeks are allowed. Create one week's working plan. Preserve these five distinct decisions in one structured response, in this reasoning order:
...
Use verified source evidence only for factual claims, retain internal hypotheses as calibration, and follow the saved communication envelope. The application owns IDs, dates, versions, influence and approvals. Application validates structure. Actual content receives factual and editorial review. Output the exact requested schema.
...
export function validateCompactStrategy(s: CompactStrategy, audiences: string[]) {
  ...
  if (new Set(focus).size !== focus.length || s.focus.secondaryAudienceKeys.length > 1) errors.push("Select distinct primary and at most one secondary audience")
