...
Validate every supplied operatingRule against its exact scope and communication element. Rules marked deterministic are also checked in code; do not weaken them. Rules marked semantic require your editorial judgment. In particular, address_form/formal must be judged from the Georgian phrasing and context rather than a pronoun regex. A material operating-rule breach makes taskFit weak and must produce a repair issue whose basis quotes the supplied directive. A caption-only rule applies only to caption, never script, on-screen text, or frame text.
