...
export async function readSubscription(pool: Pool | PoolClient, ownerId: string): Promise<Subscription | null> {
  const r = await pool.query<{ plan: SubscriptionPlan; brand_limit: number; paid_at: Date; expires_at: Date }>(
    `SELECT s.* FROM workspace_subscriptions s JOIN workspaces w ON w.id=s.workspace_id WHERE w.owner_user_id=$1`,
    [ownerId],
  )
  const s = r.rows[0]
  return s
    ? {
        plan: s.plan,
        brandLimit: s.brand_limit,
        paidAt: s.paid_at.toISOString(),
...
export async function hasSubscription(pool: Pool | PoolClient, ownerId: string) {
  return subscriptionActive(await readSubscription(pool, ownerId))
}
...
export async function purchaseSubscription(
  ...
  try {
    ...
    const previousPayment = await c.query(
      "SELECT plan FROM subscription_payments WHERE id=$1 AND workspace_id=$2",
    ...
    if (previousPayment.rowCount) {
      ...
      return await readSubscription(c, ownerId)
    }

    const custom = plan === "custom"
      ? await c.query<{ brand_limit: number }>(
          "SELECT brand_limit FROM subscription_custom_terms WHERE workspace_id=$1",
    ...
    const brands = await c.query<{ n: number }>(
      "SELECT count(*)::int n FROM brands WHERE workspace_id=$1",
      [workspace.workspaceId],
    )
    if (brands.rows[0]!.n > limit) throw Error("აირჩიეთ ტარიფი, რომელიც თქვენს ყველა ბრენდს მოიცავს.")

    const s = nextSubscriptionPeriod(await readSubscription(c, ownerId), plan, limit, billingPeriod)
    await c.query(
      `INSERT INTO workspace_subscriptions(workspace_id,plan,brand_limit,paid_at,expires_at,payment_mode)
       VALUES($1,$2,$3,$4,$5,'simulated')
       ON CONFLICT(workspace_id) DO UPDATE
       SET plan=excluded.plan,brand_limit=excluded.brand_limit,paid_at=excluded.paid_at,expires_at=excluded.expires_at`,
      [workspace.workspaceId, plan, limit, s.paidAt, s.expiresAt],
    )
    await c.query(
      "INSERT INTO subscription_payments(id,workspace_id,plan,brand_limit,paid_at,expires_at,mode) VALUES($1,$2,$3,$4,$5,$6,'simulated')",
      [paymentId, workspace.workspaceId, plan, limit, s.paidAt, s.expiresAt],
    )
    await c.query("COMMIT")
    return s
  } catch (error) {
...
export async function assertBrandCapacity(c: PoolClient, workspaceId: string, brandId: string) {
  const w = await c.query<{ owner_user_id: string }>(
    "SELECT owner_user_id FROM workspaces WHERE id=$1 FOR UPDATE",
    [workspaceId],
  )
