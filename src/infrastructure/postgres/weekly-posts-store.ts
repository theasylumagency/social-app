...
export async function beginWeeklyPosts(pool: Pool, ownerId: string, runId: string, version: number, retry = false) {
  ...
  try {
    await c.query("BEGIN")
    const r = await c.query(`SELECT r.id FROM weekly_planning_runs r WHERE ${access} AND ${currentWeeklyOperation} AND r.id=$2 AND r.version=$3 AND r.status IN ('ready','approved') AND NOT EXISTS(SELECT 1 FROM weekly_planning_runs n WHERE n.brand_id=r.brand_id AND n.week_start=r.week_start AND n.version>r.version) FOR UPDATE`, [ownerId, runId, version])
