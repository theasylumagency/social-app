...
export function WeeklyPlanningClient({ initial, initialPriority, brandId, ownerId, week, mode = "week" }: { initial: PlanningView; initialPriority: string; brandId: string; ownerId: string; week: string; mode?: "week" | "content" }) {
  ...
  return <div className="wp-app">
    ...
    {run?.payload.plan && !planningWorking ? <>{mode === "content" ? <WeeklyPostsClient run={run} batch={view.posts} assets={view.assets ?? []} onAssets={(assets) => setView((v) => ({ ...v, assets }))} onStart={() => void action("posts")} onRetry={() => void action("retry-posts")} onRepair={() => void action("repair-posts")} busy={busy} /> : <><WeeklyCadenceClient key={run.id} batch={view.posts} week={week} busy={busy} blocked={view.stale || Boolean(run.payload.review?.concerns.some((c) => c.severity === "blocking"))} onSave={(cadence) => action("cadence", note, cadence)} onStart={() => void action("posts")} onRetry={() => void action("retry-posts")} /><PlanReport run={run} /></>}{run.payload.review?.concerns.length ? <section className="wp-concerns"><h2>{blocking ? "დადასტურებამდე დასაზუსტებელია" : "რა გავითვალისწინოთ მომზადებისას"}</h2>{run.payload.review.concerns.map((c, i) => <article key={i}><span>{c.severity === "blocking" ? "დასაზუსტებელია" : "გასათვალისწინებელია"}</span><p>{c.message}</p>{c.directionKeys.length ? <small>მიმართულებები: {c.directionKeys.map((key) => key.slice(1)).join(", ")}</small> : null}</article>)}</section> : null}<section className="wp-review-actions"><div><p className="wp-eyebrow">{contentApproved ? "გადაწყვეტილება შენახულია" : "თქვენი ხედვა გეგმას ასრულებს"}</p><h2>{contentApproved ? "ამ კვირის გეგმა დადასტურებულია." : "კვირის მიმართულება და კონტენტი განვიხილოთ."}</h2><p>{contentApproved ? "გეგმა შენახულია. გამოსახულებების დამატება შეგიძლიათ კონტენტის ტაბში; გამოქვეყნება ჯერ არ დაწყებულა." : "დადასტურება შეინახავს გეგმასა და მომზადებულ ტექსტებს. გამოქვეყნებისთვის ჯერ ვიზუალები და ანგარიშების დაკავშირებაა საჭირო."}</p></div><div>{mode === "content" && ["ready", "approved"].includes(run.status) && !contentApproved ? <button className="wp-button" disabled={busy || blocking || view.stale || revising || !postsReady} onClick={() => void action("approve")}>{busy ? "ინახება…" : "ვადასტურებ კვირის გეგმას →"}</button> : null}{mode === "week" ? <Link className="wp-button" href={`/workspace/content?week=${week}`}>პოსტების განხილვა →</Link> : <Link className="wp-text-link" href={`/workspace?week=${week}`}>კვირის მიზანი და რაოდენობის შეცვლა ↗</Link>}<button className="wp-button wp-button-outline" disabled={busy || planningWorking} onClick={() => setRevising(!revising)}>{revising ? "დაზუსტების დახურვა" : "გეგმის დაზუსტება"}</button></div></section></> : null}
