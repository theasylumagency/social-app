...
import { currentWeek, displayDate } from "../../application/dashboard/model"
import { type PlanningRun, type PlanningView } from "../../blueprints/social/weekly-planning/model"
...
function PlanReport({ run }: { run: PlanningRun }) {
  ...
  return <div className="wp-report">
    <section className="ws-card wp-evidence"><h3>რა მონაცემს დაეყრდნო ეს კვირა</h3>{p.evidence?.map((e) => <article key={e.week}><strong>{e.week} · {e.availability === "available" ? "დაკვირვებები ხელმისაწვდომია" : "შედეგები უცნობია"}</strong>{e.execution.map((v) => <p key={v}>{v}</p>)}{e.observations.map((o, i) => <p key={i}>{o.observation} · წყარო: {o.source}</p>)}{e.unknowns.map((v) => <p key={v}>{v}</p>)}</article>) ?? <p>ძველი გეგმა: შედეგების გამოყენების ჩანაწერი არ არის.</p>}</section>
    <section className="wp-objective"><div className="wp-kicker"><span className="wp-dot" /><span>{statusLabels[run.status]}</span><span>ვერსია {run.version}</span></div><p className="wp-eyebrow">ამ კვირის მთავარი ამოცანა</p><h2>{plan.objective.objective}</h2><p className="wp-objective-reason">{plan.objective.rationale}</p><div className="wp-goal-link"><span>ემსახურება ბრენდის მიზანს</span>{strategic ? <strong>{strategic.objective}</strong> : <span>ისტორიული გეგმა · მოქმედი სოციალური სტრატეგია ჯერ არ იყო დადგენილი.</span>}{!strategic ? alignedGoals.map((g) => <strong key={g.id}>{g.title}</strong>) : null}</div></section>
...
export function WeeklyPlanningClient({ initial, initialPriority, brandId, ownerId, week, mode = "week" }: { initial: PlanningView; initialPriority: string; brandId: string; ownerId: string; week: string; mode?: "week" | "content" }) {
  ...
  if (!run && week !== currentWeek()) return <section className="wp-start"><h2>ამ კვირის გეგმა არ არის შენახული.</h2><p>დეტალურ გეგმას მხოლოდ მიმდინარე კვირისთვის ვამზადებთ.</p><Link className="wp-button" href="/workspace">მიმდინარე კვირაზე დაბრუნება →</Link></section>
  return <div className="wp-app">
    {error ? <p className="wp-error" role="alert">{error}</p> : null}
    {!run && mode === "content" ? <section className="wp-start"><h2>ამ კვირის კონტენტი ჯერ არ მომზადებულა.</h2><p>ჯერ კვირის გეგმა შექმენით. პოსტების მომზადება შემდეგ ავტომატურად დაიწყება.</p><Link className="wp-button" href={`/workspace?week=${week}`}>კვირის გეგმის შექმნა →</Link></section> : null}
    {!run && mode === "week" ? <section className="wp-start"><div><p className="wp-eyebrow">ბრენდის ცოდნიდან — ამ კვირის არჩევანამდე</p><h2>მთელი კვირის პოსტები.<br />თქვენს ბიზნესზე მორგებული.</h2><p>შემოგთავაზებთ, რამდენი პოსტი გჭირდებათ, სად და რატომ გამოქვეყნდეს, რა ეწეროს და როგორი გამოსახულება მოუხდეს. თქვენ ნახავთ, დააზუსტებთ და დაადასტურებთ.</p><div className="wp-foundation-chips"><span>{view.basis.payload.landscape?.entries.filter((e) => e.influence !== "none").length} აუდიტორიული სიტუაცია</span><span>ერთი შეთანხმებული სოციალური მიზანი</span><span>შენახული კომუნიკაციის ჩარჩო</span></div><Link className="wp-text-link" href="/workspace/brand">რას ვეყრდნობით ↗</Link></div><form onSubmit={(e) => { e.preventDefault(); void action("start") }}><label htmlFor="wp-priority">ამ კვირაში რამეს განსაკუთრებული ყურადღება სჭირდება?</label><textarea id="wp-priority" rows={5} maxLength={1200} value={priority} onChange={(e) => setPriority(e.target.value)} placeholder="მაგ. ახალი შეთავაზება გვაქვს, ან ხშირად გვიმეორებენ ერთსა და იმავე კითხვას…" /><p>სურვილისამებრ. თუ პრიორიტეტს არ მიუთითებთ, კვირას შეთანხმებული სოციალური მიზნის მიხედვით დავგეგმავთ.</p><button className="wp-button" disabled={busy}>{busy ? "ვიწყებთ…" : "შემომთავაზე კვირის გეგმა →"}</button><small>ჯერ გეგმას განიხილავთ და დაადასტურებთ.</small></form></section> : null}
    ...
    {run?.payload.plan && !planningWorking ? <>{mode === "content" ? <WeeklyPostsClient run={run} batch={view.posts} assets={view.assets ?? []} onAssets={(assets) => setView((v) => ({ ...v, assets }))} onStart={() => void action("posts")} onRetry={() => void action("retry-posts")} onRepair={() => void action("repair-posts")} busy={busy} readOnly={week !== currentWeek()} /> : <><WeeklyCadenceClient key={run.id} batch={view.posts} week={week} busy={busy} blocked={week !== currentWeek() || view.stale || Boolean(run.payload.review?.concerns.some((c) => c.severity === "blocking"))} onSave={(cadence) => action("cadence", note, cadence)} onStart={() => void action("posts")} onRetry={() => void action("retry-posts")} /><PlanReport run={run} /></>}{run.payload.review?.concerns.length ? <section className="wp-concerns"><h2>{blocking ? "დადასტურებამდე დასაზუსტებელია" : "რა გავითვალისწინოთ მომზადებისას"}</h2>{run.payload.review.concerns.map((c, i) => <article key={i}><span>{c.severity === "blocking" ? "დასაზუსტებელია" : "გასათვალისწინებელია"}</span><p>{c.message}</p>{c.directionKeys.length ? <small>მიმართულებები: {c.directionKeys.map((key) => key.slice(1)).join(", ")}</small> : null}</article>)}</section> : null}<section className="wp-review-actions"><div><p className="wp-eyebrow">{contentApproved ? "გადაწყვეტილება შენახულია" : "თქვენი ხედვა გეგმას ასრულებს"}</p><h2>{contentApproved ? "ამ კვირის გეგმა დადასტურებულია." : "კვირის მიმართულება და კონტენტი განვიხილოთ."}</h2><p>{contentApproved ? "გეგმა შენახულია. გამოსახულებების დამატება შეგიძლიათ კონტენტის ტაბში; გამოქვეყნება ჯერ არ დაწყებულა." : "დადასტურება შეინახავს გეგმასა და მომზადებულ ტექსტებს. გამოქვეყნებისთვის ჯერ ვიზუალები და ანგარიშების დაკავშირებაა საჭირო."}</p></div><div>{week === currentWeek() && mode === "content" && ["ready", "approved"].includes(run.status) && !contentApproved ? <button className="wp-button" disabled={busy || blocking || view.stale || revising || !postsReady} onClick={() => void action("approve")}>{busy ? "ინახება…" : "ვადასტურებ კვირის გეგმას →"}</button> : null}{mode === "week" ? <Link className="wp-button" href={`/workspace/content?week=${week}`}>პოსტების განხილვა →</Link> : <Link className="wp-text-link" href={`/workspace?week=${week}`}>კვირის მიზანი და რაოდენობის შეცვლა ↗</Link>}<button className="wp-button wp-button-outline" disabled={week !== currentWeek() || busy || planningWorking} onClick={() => setRevising(!revising)}>{revising ? "დაზუსტების დახურვა" : "გეგმის დაზუსტება"}</button></div></section></> : null}
