...
export function WeeklyPlanningClient({ initial, initialPriority, brandId, ownerId, week }: { initial: PlanningView; initialPriority: string; brandId: string; ownerId: string; week: string }) {
  ...
  async function action(kind: "start" | "revise" | "approve" | "retry" | "posts" | "retry-posts" | "repair-posts", revisionNote = note) {
  ...
  return <div className="wp-app">
    ...
    {run?.payload.plan && !planningWorking ? <><WeeklyPostsClient run={run} batch={view.posts} assets={view.assets ?? []} onAssets={(assets) => setView((v) => ({ ...v, assets }))} onStart={() => void action("posts")} onRetry={() => void action("retry-posts")} onRepair={() => void action("repair-posts")} busy={busy} /><details className="fp-strategy wp-details"><summary>სტრატეგია და დასაბუთება — რის მიხედვით შევარჩიეთ პოსტები</summary><PlanReport run={run} /></details>{run.payload.review?.concerns.length ? <section className="wp-concerns"><h2>{blocking ? "დადასტურებამდე დასაზუსტებელია" : "რა გავითვალისწინოთ მომზადებისას"}</h2>{run.payload.review.concerns.map((c, i) => <article key={i}><span>{c.severity === "blocking" ? "დასაზუსტებელია" : "გასათვალისწინებელია"}</span><p>{c.message}</p>{c.directionKeys.length ? <small>მიმართულებები: {c.directionKeys.map((key) => key.slice(1)).join(", ")}</small> : null}</article>)}</section> : null}<section className="wp-review-actions"><div><p className="wp-eyebrow">{contentApproved ? "გადაწყვეტილება შენახულია" : "თქვენი ხედვა გეგმას ასრულებს"}</p><h2>{contentApproved ? "ამ კვირის გეგმა დადასტურებულია." : "ეს პოსტები დავიტოვოთ ამ კვირისთვის?"}</h2><p>{contentApproved ? "გეგმა შენახულია. გამოსახულებების დამატება შეგიძლიათ აქვე; გამოქვეყნება ჯერ არ დაწყებულა." : "დადასტურება შეინახავს გეგმასა და მომზადებულ ტექსტებს. გამოქვეყნებისთვის ჯერ ვიზუალები და ანგარიშების დაკავშირებაა საჭირო."}</p></div><div>{["ready", "approved"].includes(run.status) && !contentApproved ? <button className="wp-button" disabled={busy || blocking || view.stale || revising || !postsReady} onClick={() => void action("approve")}>{busy ? "ინახება…" : "ვადასტურებ კვირის გეგმას →"}</button> : null}<button className="wp-button wp-button-outline" disabled={busy || working} onClick={() => setRevising(!revising)}>{revising ? "დაზუსტების დახურვა" : "გეგმის დაზუსტება"}</button></div></section></> : null}
