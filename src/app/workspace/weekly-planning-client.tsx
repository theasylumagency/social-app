...
function PlanReport({ run }: { run: PlanningRun }) {
  ...
  return <div className="wp-report">
    ...
    <section className="wp-executive-focus"><h3>ვისთან გვინდა პროგრესი</h3><p>{primary?.name ?? "აუდიტორიის დეტალი სრულ გეგმაში ნახეთ"}</p>{p.review?.progressSignals.length ? <><h3>რას დავაკვირდებით</h3><ul>{p.review.progressSignals.map((signal) => <li key={signal}>{signal}</li>)}</ul></> : null}</section>
