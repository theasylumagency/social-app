...
export function sectionUrl(section: DashboardSection) { return section === "overview" ? "/workspace" : `/workspace/${section}` }

export function WorkspaceShell({ section, brands, brand, user, children, week }: {
  ...
  return <div className="ws-shell">
    ...
    <aside className="ws-sidebar">
      <Link className="ws-logo" href="/workspace" aria-label="UNDA Social მთავარი"><span className="ws-logo-mark">u<span>·</span></span><span>UNDA<small>SOCIAL</small></span></Link>
      <BrandSwitcher brands={brands.map(({ id, name }) => ({ id, name }))} active={brand.id} />
      <div className="ws-nav-caption">თქვენი სოციალური გუნდი</div>
