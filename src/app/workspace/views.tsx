...
export function BrandView({ brand, sources, view, dossier, history = [] }: { brand: DashboardBrand; sources: DashboardSource[]; view: string; dossier?: BrandDossier | null; history?: DossierHistoryEntry[] }) {
  ...
  if (active === "knowledge" && dossier) return <><nav className="ws-tabs"><Link href="/workspace/brand" aria-current="page">ბრენდის ხედვა</Link><Link href="/workspace/brand?view=sources">წყაროები</Link><Link href="/workspace/brand?view=history">ისტორია</Link></nav><BrandDossierView key={dossier.sessionId} payload={dossier.payload} refineHref={`/onboarding?brand=${encodeURIComponent(brand.id)}`} /></>
