import type { DossierHistoryEntry } from "../../infrastructure/postgres/brand-discovery-store"
...
export function WeekView({ brand, week, today, brief, planning, ownerId }: { brand: DashboardBrand; sources: DashboardSource[]; week: string; today: string; brief: WeeklyBrief; planning: PlanningView; ownerId: string }) {
  return <><PageHeading eyebrow="ბრენდის ხედვიდან — კვირის გადაწყვეტილებამდე" title={week === today ? "თქვენი კვირის გეგმა" : "კვირის სამუშაო სივრცე"} description={`${brand.name} · ამოცანა, მისი საფუძველი და კონტენტის მიმართულებები`}><div className="ws-week-picker"><Link href={`/workspace/week?week=${shiftWeek(week, -1)}`} aria-label="წინა კვირა"><Icon name="chevron" style={{ transform: "rotate(180deg)" }} /></Link><span><Icon name="week" />{weekLabel(week)}</span><Link href={`/workspace/week?week=${shiftWeek(week, 1)}`} aria-label="შემდეგი კვირა"><Icon name="chevron" /></Link></div></PageHeading>{week !== today ? <Link className="ws-back-current" href="/workspace/week">მიმდინარე კვირაზე დაბრუნება <Icon name="arrow" /></Link> : null}<WeeklyPlanningClient key={`${brand.id}:${week}`} initial={planning} initialPriority={brief?.objective ?? ""} brandId={brand.id} week={week} ownerId={ownerId} /></>
}

export function ContentView({ planning, brand, ownerId, week }: { planning: PlanningView; brand: DashboardBrand; ownerId: string; week: string }) {
  return <><PageHeading eyebrow="ტექსტები · გამოსახულებები · განხილვა" title="თქვენი კონტენტი" description={`${brand.name} · კვირის გეგმის მიხედვით მომზადებული პოსტები`}><div className="ws-week-picker"><Link href={`/workspace/content?week=${shiftWeek(week, -1)}`} aria-label="წინა კვირა"><Icon name="chevron" style={{ transform: "rotate(180deg)" }} /></Link><span><Icon name="week" />{weekLabel(week)}</span><Link href={`/workspace/content?week=${shiftWeek(week, 1)}`} aria-label="შემდეგი კვირა"><Icon name="chevron" /></Link></div></PageHeading><WeeklyPlanningClient key={`${brand.id}:${week}:content`} initial={planning} initialPriority="" brandId={brand.id} ownerId={ownerId} week={week} mode="content" /></>
}
...
export function BrandView({ brand, sources, view, dossier, history = [] }: { brand: DashboardBrand; sources: DashboardSource[]; view: string; dossier?: BrandDossier | null; history?: DossierHistoryEntry[] }) {
  ...
  if (active === "history" && history.length) return <><PageHeading eyebrow="ბრენდის ისტორია" title="როგორ იცვლებოდა ჩვენი გაგება" description="დადასტურებული ვერსიები ინახება, რომ ცვლილება და მისი საფუძველი ყოველთვის ჩანდეს." /><Link className="ws-text-link" href="/workspace/brand">ბრენდის მიმდინარე ხედვა ↗</Link>{history.map((entry, i) => <section className="ws-card bd-history-entry" key={entry.sessionId}><p className="bd-eyebrow">{i === 0 ? "მოქმედი საფუძველი" : "წინა ვერსია"} · <time dateTime={entry.confirmedAt}>{displayDate(entry.confirmedAt, { year: "numeric", hour: "2-digit", minute: "2-digit" })}</time></p><p>{entry.summary}</p><details className="bd-detail"><summary>შეთავაზებები და არჩეული მიზნები</summary><h3>შეთავაზებები</h3><ul>{entry.offers.map((offer) => <li key={offer}>{offer}</li>)}</ul><h3>მიზნები</h3><ul>{entry.goals.map((goal) => <li key={goal}>{goal}</li>)}</ul></details></section>)}</>
  if (active === "knowledge" && dossier) return <><nav className="ws-tabs"><Link href="/workspace/brand" aria-current="page">ბრენდის ხედვა</Link><Link href="/workspace/brand?view=sources">წყაროები</Link><Link href="/workspace/brand?view=history">ისტორია</Link></nav><BrandBrief key={dossier.sessionId} dossier={dossier} brandId={brand.id} /></>
  return <>{!dossier ? <section className="bd-legacy-banner"><div><h2>გავიცნოთ თქვენი ბრენდი უფრო ღრმად.</h2><p>ბიზნესის ახალი გააზრება, აუდიტორიები, მიზნები და კომუნიკაციის ჩარჩო.</p></div><Link className="bd-button" href={`/onboarding?brand=${encodeURIComponent(brand.id)}`}>ბრენდის გაცნობა ↗</Link></section> : null}<PageHeading eyebrow="თქვენი ბრენდის საფუძველი" title={brand.name} description="რა ვიცით, საიდან ვიცით და რას ვეყრდნობით." /><nav className="ws-tabs ws-brand-tabs" aria-label="ბრენდის ინფორმაცია">{[{ id: "knowledge", label: "ბრენდის ცოდნა" }, { id: "sources", label: "წყაროები" }, { id: "history", label: "ისტორია" }].map((item) => <Link href={`/workspace/brand?view=${item.id}`} key={item.id} aria-current={active === item.id ? "page" : undefined}>{item.label}{item.id === "sources" ? <span className="ws-count">{sources.length}</span> : null}</Link>)}</nav>
    {active === "sources" ? <section className="ws-card"><div className="ws-card-heading"><h2>საიდან ვიცით ბრენდის შესახებ</h2></div><SourcesList sources={sources} /><p className="ws-card-note">წყაროს შენახული ვერსია გამოყენებულია ბრენდის საწყისი ინფორმაციისთვის. ავტომატური ხელახალი შემოწმება ჯერ არ არის ჩართული.</p></section> : active === "history" ? <section className="ws-card ws-history"><span className="ws-history-check"><Icon name="check" /></span><div><time dateTime={brand.createdAt}>{displayDate(brand.createdAt, { year: "numeric" })}</time><h2>შეიქმნა ბრენდის საწყისი საფუძველი</h2><p>თქვენ მიერ გადამოწმებული ინფორმაცია შენახულია. საფუძველი ეყრდნობა {sources.length} წყაროს.</p><Link className="ws-text-link" href="/workspace/brand?view=sources">წყაროების ნახვა <Icon name="arrow" /></Link></div></section> : <>
      ...
      <details className="brief-details"><summary>ბრენდის შენახული დეტალები</summary><div className="ws-knowledge-grid"><KnowledgeSection title="რას ვთავაზობთ" items={knowledgeList(k, "offerPrimaryServices")} fallback="მომსახურება ჯერ არ არის მითითებული." /><KnowledgeSection title="ვის ვესაუბრებით" items={knowledgeList(k, "audiencePrimarySegments")} fallback="აუდიტორია ჯერ არ არის დაზუსტებული. კონკრეტული ასაკი ან ინტერესები არ არის ნავარაუდევი." /><KnowledgeSection title="ხმა და სტილი" items={knowledgeList(k, "voicePrimaryTone").map((tone) => toneLabels[tone] ?? tone)} fallback="მარტივი, მშვიდი და პროფესიონალური კომუნიკაცია." defaultVoice /><KnowledgeSection title="საკომუნიკაციო მიზნები" items={knowledgeList(k, "contentGoals")} fallback="კონკრეტული კვირის პრიორიტეტის დამატება კვირის გვერდიდან შეგიძლიათ." /><KnowledgeSection title="შეზღუდვები" items={knowledgeList(k, "constraintsSensitiveTopics")} fallback="დამატებითი შეზღუდვები არ არის მითითებული." /><KnowledgeSection title="კონტენტის ენა" items={knowledgeList(k, "identityLanguages").map((language) => language === "ka" ? "ქართული" : language === "en" ? "ინგლისური" : language)} fallback="ენა ჯერ არ არის მითითებული." /></div></details>
...
export function ConnectionsView({ sources, ...connections }: { sources: DashboardSource[]; brandId: string; accounts: ConnectionAccountView[]; available: boolean; intentId: string; outcome: string }) {
  ...
  return <><PageHeading eyebrow="ბრენდის კავშირები" title="სოციალური ანგარიშები" description="სოციალური ანგარიშები და წვდომა თქვენს წყაროებზე." />
