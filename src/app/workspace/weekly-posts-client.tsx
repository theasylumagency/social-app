...
function PostCard({ run, post, index, copy, assets, onAssets, readOnly = false, preferredChannel }: { run: PlanningRun; post: PostOutline; index: number; copy: PostCopy | undefined; assets: PostAsset[]; onAssets: (assets: PostAsset[]) => void; readOnly?: boolean; preferredChannel?: PostChannel | undefined }) {
...
export function WeeklyPostsClient({ run, batch, assets, onAssets, onStart, onRetry, onRepair, busy, readOnly = false }: { run: PlanningRun; batch: PostsBatch | null | undefined; assets: PostAsset[]; onAssets: (assets: PostAsset[]) => void; onStart: () => void; onRetry: () => void; onRepair?: () => void; busy: boolean; readOnly?: boolean }) {
  ...
  return <div className="fp-app">
    ...
    {visible.map(({ post, index }) => <PostCard key={`${run.id}:${index}:${filter}`} run={run} post={post} index={index} copy={batch.payload.copies[`p${index + 1}`]} assets={assets} onAssets={onAssets} readOnly={readOnly} preferredChannel={filter === "all" ? undefined : filter} />)}
