...
import type { NoteContext, NoteEntry } from "../../application/contextual-notes/model"
import { VoiceNoteInput } from "./voice-note-input"

type Selection = { postKey: string; channel: "facebook" | "instagram"; runId: string; title: string; postVersion: string }
...
export function PostNoteButton({ runId, postKey, channel, title, postVersion }: Selection) {
  const select = useContext(NotesContext)
  return select ? <button type="button" className="cn-post-button" onClick={() => select({ runId, postKey, channel, title, postVersion })}>✎ ამ პოსტზე შენიშვნა</button> : null
}
...
export function ContextualNotes({ brandId, brandName, section, sectionLabel, week, voiceAvailable, children }: {
  ...
  const context: NoteContext = { brandId, section, week, postKey: selection?.postKey ?? null, channel: selection?.channel ?? null, runId: selection?.runId ?? null, postVersion: selection?.postVersion ?? null }
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!controller.signal.aborted) { setNotes(data.notes); setLoading(false) }
  ...
  async function action(kind: "submit" | "confirm" | "dismiss" | "undo", note?: NoteEntry) {
    ...
    try {
      ...
      if (next.status === "applied" || next.status === "reverted") { router.refresh(); window.dispatchEvent(new Event("unda:notes-changed")) }
  ...
  function retry(note: NoteEntry) {
    ...
    setSelection(note.context.postKey && note.context.channel && note.context.runId ? { postKey: note.context.postKey, channel: note.context.channel, runId: note.context.runId, title: `პოსტი ${note.context.postKey.slice(1)}`, postVersion: note.context.postVersion ?? "" } : null)
  ...
  return <NotesContext.Provider value={selectPost}>
    <section className={`cn-panel ${collapsed ? "is-collapsed" : ""}`} id="contextual-notes" aria-labelledby="cn-title">
      ...
      <div id="cn-body" hidden={collapsed}>
        ...
        {selection ? <div className="cn-selection"><span><strong>{selection.title}</strong> · {selection.channel === "facebook" ? "Facebook" : "Instagram"}</span><button type="button" disabled={busy || voiceBusy || !!text.trim()} onClick={() => { setSelection(null); requestId.current = null }} aria-label="პოსტის არჩევის გაუქმება">×</button></div> : null}
