...
export function WeeklyCadenceClient({ batch, week, busy, blocked, onSave, onStart, onRetry }: { batch: PostsBatch | null | undefined; week: string; busy: boolean; blocked: boolean; onSave: (cadence: PostCadence) => Promise<void>; onStart: () => void; onRetry: () => void }) {
  ...
  return <section className="wp-cadence" aria-label="კვირის პოსტების რაოდენობა">
    <div className="wp-cadence-heading"><div><p className="wp-eyebrow">როგორ განვახორციელებთ გეგმას</p><h2>კვირის რიტმი თქვენი ბიზნესისთვის</h2><p>{outline ? outline.cadenceReason : "თქვენი ბიზნესისა და კვირის მიზნის მიხედვით პოსტების რაოდენობასა და არხებს ვარჩევთ."}</p></div><Link className="wp-button wp-button-outline" href={`/workspace/content?week=${week}`}>კონტენტის ნახვა →</Link></div>
    {outline ? <div className="brief-tags">{(["facebook", "instagram"] as const).map((channel) => <span key={channel}>{channel === "facebook" ? "Facebook" : "Instagram"} · {saved[channel]} პოსტი</span>)}</div> : null}
    <details className="brief-details"><summary>რაოდენობის შეცვლა და არხების დასაბუთება</summary><form onSubmit={async (e) => { e.preventDefault(); await onSave(counts) }}>
    ...
    {working ? <p className="fp-progress" role="status"><span className="fp-pulse" />{batch.step === "outline" ? "პოსტების განრიგს ვამზადებთ." : batch.step === "writing" ? `ფონში ტექსტები მზადდება: ${Object.keys(batch.payload.copies).length} / ${outline?.posts.length ?? 0}.` : "პოსტების ტექსტების ხარისხს ვამოწმებთ."} გეგმის კითხვა შეგიძლიათ გააგრძელოთ.</p> : null}
    ...
    {batch?.status === "ready" ? <p className="wp-caption">{batch.payload.review?.issues.some((i) => i.severity === "blocking") ? "ტექსტებში დასაზუსტებელი საკითხებია — იხილეთ კონტენტის ტაბში." : batch.approvedAt ? "კონტენტი დადასტურებულია. გამოსაქვეყნებელი ვიზუალები და განრიგი კონტენტის გვერდზე ნახეთ." : "კონტენტი მზადაა განსახილველად. ტექსტები და გამოსახულებები კონტენტის ტაბში ნახეთ."}</p> : null}
  </section>
}
