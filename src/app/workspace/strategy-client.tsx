...
export function StrategyClient({ brandId, initial }: { brandId: string; initial: StrategyView }) {
  ...
  const [reason, setReason] = useState<RevisionReason>(initial.latest?.status === "approved" ? "performanceEvidence" : "founderFeedback")
  ...
  return <div className="strategy-client"><div className="ws-heading"><p className="eyebrow">ბრენდის გაცნობა → სოციალური გარემო → შეთანხმებული სტრატეგია</p><h1>მიზანი, რომელიც კვირებს აერთიანებს.</h1></div>
    {!latest ? <section className="ws-card"><h2>UNDA შემოგთავაზებთ სოციალურ მიზანს.</h2><p>შევაფასებთ ხელმისაწვდომ საჯარო გვერდებს, ავირჩევთ არხების როლებს და ჩამოვაყალიბებთ სტრატეგიასა და პროგრესის ნიშნებს.</p>{initial.legacy ? <p>ძველი გეგმები შენარჩუნებულია. მათი მიზნები ავტომატურად ახალ სტრატეგიად არ გადაგვაქვს.</p> : null}<button className="ws-button ws-button-green" disabled={busy} onClick={() => void act("propose")}>კვლევა და რეკომენდაციის მომზადება →</button></section> : null}
    ...
    {latest?.status === "failed" ? <section className="ws-card"><p>{latest.error}</p><button className="ws-button ws-button-green" disabled={busy} onClick={() => void act("retry")}>ხელახლა ცდა</button></section> : null}
    {latest?.payload.proposal ? <StrategyReport strategy={latest} /> : null}
    {latest?.status === "proposed" || latest?.status === "approved" ? <section className="ws-card strategy-actions">{latest.status === "proposed" ? <button className="ws-button ws-button-green" disabled={busy} onClick={() => void act("approve")}>ვეთანხმები სტრატეგიულ მიზანს →</button> : <Link className="ws-button ws-button-green" href="/workspace/week">მიმდინარე კვირის დაგეგმვა →</Link>}<button className="ws-button ws-button-outline" disabled={busy} onClick={() => { setReason(latest.status === "approved" ? "performanceEvidence" : "founderFeedback"); setEditing(!editing) }}>{view.active ? "სტრატეგიის ცვლილების საფუძველი" : "არ ვეთანხმები / კომენტარი"}</button></section> : null}
    {editing ? <form className="ws-card strategy-form" onSubmit={(e) => { e.preventDefault(); void act("revise") }}><label>ცვლილების მიზეზი<select value={reason} onChange={(e) => setReason(e.target.value as RevisionReason)}>{Object.entries(reasons).filter(([key]) => latest?.status !== "approved" || key !== "founderFeedback").map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label><label>რას უნდა დაეყრდნოს ახალი რეკომენდაცია<textarea required minLength={10} maxLength={3000} rows={5} value={comment} onChange={(e) => setComment(e.target.value)} /></label><button className="ws-button ws-button-green" disabled={busy}>კომენტარის გათვალისწინება და ახალი რეკომენდაცია</button></form> : null}
