...
import type { BillingPeriod, SubscriptionPlan } from "../../../application/subscriptions/policy"
...
export async function POST(request: Request) {
  ...
  if (process.env.NODE_ENV === "production" && process.env.BILLING_MODE !== "simulated") {
    return Response.json({ message: "სატესტო გადახდა ამ გარემოში გამორთულია." }, { status: 403 })
  }
  ...
  try {
    ...
    if (
      !body ||
      !isDiscoveryId(body.paymentId) ||
      !["solo", "studio", "agency", "custom"].includes(body.plan) ||
      !["monthly", "annual"].includes(body.billingPeriod)
    ) throw Error()
    ...
    return Response.json(
      { message: error instanceof Error && /[ა-ჰ]/u.test(error.message) ? error.message : "გადახდა ვერ შეინახა. სცადეთ ხელახლა." },
      { status: 422 },
    )
  }
}
