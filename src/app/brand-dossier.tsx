...
export function BrandDossierView({ payload: p, busy = false, onFeedback, onConfirm, refineHref, draftKey }: Props) {
  ...
  return <div className="bd-dossier">
    ...
    {onConfirm ? <footer className="bd-confirm"><div><p className="bd-eyebrow">ეს იქნება ჩვენი სამუშაო საფუძველი</p><h2>რამდენად გეცნობათ თქვენი ბიზნესი?</h2><p>დადასტურება შეინახავს ბრენდის საქმიანობასა და კომუნიკაციის საფუძველს. აუდიტორიის ჰიპოთეზები და თქვენი პასუხები დამოუკიდებლად დარჩება.</p></div><div className="bd-confirm-actions"><label htmlFor="bd-content-language">კონტენტის ენა<select id="bd-content-language" value={language} onChange={(e) => setLanguage(e.target.value as "ka" | "en")}><option value="ka">ქართული</option><option value="en">English</option></select></label><button type="button" className="bd-button" disabled={busy || feedbackDirty || !!businessNote.trim()} onClick={() => void onConfirm(goalIds, language)}>{busy ? "ინახება…" : "ვადასტურებ ბრენდის საფუძველს →"}</button>{businessNote.trim() ? <small>ჯერ ბიზნესის დაზუსტება გაითვალისწინეთ ან წაშალეთ მონახაზი.</small> : feedbackDirty ? <small>ჯერ თქვენი აუდიტორიული პასუხები გაითვალისწინეთ.</small>  : <small>შემდგომი დაზუსტება ყოველთვის შეგეძლებათ.</small>}</div></footer> : null}
  </div>
}
