...
export async function runWeeklyPlanning(pool: Pool, ownerId: string, id: string, budgetMs = 290_000) {
  ...
  while (Date.now() < deadline - 185_000) {
    ...
    if (!claim) return
    try {
      const next = await advanceWeeklyPlanning(claim.run, createBrandReasoner((run) => recordPlanningModelRun(pool, id, run), { model: process.env.OPENAI_PLANNING_MODEL ?? process.env.OPENAI_BRAND_MODEL ?? "gpt-5.6-sol" }))
      if (!await finishPlanningStep(pool, claim.run, claim.token, next.payload, next.step) || next.step === "ready") return
    } catch (error) {
