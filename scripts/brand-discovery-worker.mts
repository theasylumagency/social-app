...
try {
  do {
    ...
    const posts = await pool.query<{ id: string; owner_user_id: string }>(`SELECT r.id,r.owner_user_id FROM weekly_post_batches p JOIN weekly_planning_runs r ON r.id=p.run_id JOIN auth_user u ON u.id=r.owner_user_id WHERE u."emailVerified"=true AND r.status IN ('ready','approved') AND NOT EXISTS(SELECT 1 FROM weekly_planning_runs n WHERE n.brand_id=r.brand_id AND n.week_start=r.week_start AND n.version>r.version) AND (p.status='queued' OR (p.status='running' AND p.lease_until<now())) ORDER BY p.updated_at LIMIT 2`)
    const results = await Promise.allSettled([...pending.rows.map((job) => runBrandDiscovery(pool, job.owner_user_id, job.id)), ...weekly.rows.map((job) => runWeeklyPlanning(pool, job.owner_user_id, job.id)), ...posts.rows.map((job) => runWeeklyPosts(pool, job.owner_user_id, job.id))])
