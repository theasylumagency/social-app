...
try {
  do {
    ...
    const results = await Promise.allSettled([...strategies.rows.map((job) => runSocialStrategy(pool, job.owner_user_id, job.id)), ...pending.rows.map((job) => runBrandDiscovery(pool, job.owner_user_id, job.id)), ...weekly.rows.map((job) => runWeeklyPlanning(pool, job.owner_user_id, job.id)), ...posts.rows.map((job) => runWeeklyPosts(pool, job.owner_user_id, job.id))])
